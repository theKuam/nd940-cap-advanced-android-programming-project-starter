...
class RepresentativeViewHolder(val binding: ItemRepresentativeBinding) :
    ...
    fun bind(item: Representative) {
        ...
        item.official.urls?.let { urlList ->
            showWWWLinks(urlList)
        }
        item.official.channels?.let { channelList ->
            showSocialLinks(channelList)
        }
    ...
    companion object {
        fun from(parent: ViewGroup): RepresentativeViewHolder {
            ...
            val binding = DataBindingUtil.inflate<ItemRepresentativeBinding>(
                layoutInflater,
                R.layout.item_representative,
                parent,
                false
            )
    ...
    private fun showSocialLinks(channels: List<Channel>) {
        ...
        if (!facebookUrl.isNullOrBlank()) {
            enableLink(binding.ivFacebookUrl, facebookUrl)
        }

        val twitterUrl = getTwitterUrl(channels)
        if (!twitterUrl.isNullOrBlank()) {
            enableLink(binding.ivTwitterUrl, twitterUrl)
        }
    }

    private fun showWWWLinks(urls: List<String>) {
        enableLink(binding.ivWebsiteUrl, urls.first())
    }
